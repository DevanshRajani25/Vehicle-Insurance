'''
This is final mega componenet for data validation task which will fetch multiple methods 
for different validations. Steps like:

    1) read train & test csv file data
    2) check if no. of columns in train file is similar to schema file
        if len(fetched_data) == len(schema_columns):
            error_msg = ''
        else:
            it will log and also give error like this is missing
    3) same process for test dataset
    4) If everything is okay then give positive results in all artifact folder
'''
